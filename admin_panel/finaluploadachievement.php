...
$target_dir="img/upload/achievements/";
chmod($target_dir,0777);

$count = count($_FILES["fileToUploadAchievement"]["name"]);
$filearray = [];
$messageErr = [];

for ($i=0; $i < $count ; $i++)
{ 
	$target_file=$target_dir . time(). basename($_FILES["fileToUploadAchievement"]["name"][$i]);
	$uploadok=1;
	...
	{
		if($_FILES["fileToUploadAchievement"]["name"][$i]=="")
		{
			echo "No File is Selected.";
			exit();
		}
		$check=getimagesize($_FILES["fileToUploadAchievement"]["tmp_name"][$i]);
		if($check!==false)
		{
	...
	//check file size
	if($_FILES["fileToUploadAchievement"]["size"][$i]>500000)
	{
	...
	{
		if(move_uploaded_file($_FILES["fileToUploadAchievement"]["tmp_name"][$i], $target_file))
		{
			chmod($target_file,0777);
			echo "The file ". basename($_FILES["fileToUploadAchievement"]["name"][$i]). " has been uploaded.";
