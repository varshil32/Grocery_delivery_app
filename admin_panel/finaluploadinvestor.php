<?php
$target_dir="img/upload/investors/";
chmod($target_dir,0777);
$count = count($_FILES["fileToUploadInvestor"]["name"]);
$filearray = [];
$messageErr = [];
for ($i=0; $i < $count ; $i++)
{ 
	$target_file=$target_dir . time(). basename($_FILES["fileToUploadInvestor"]["name"][$i]);
	$uploadok=1;
	...
	{
		if($_FILES["fileToUploadInvestor"]["name"][$i]=="")
		{
			echo "No File is Selected.";
			exit();
		}
		$check=getimagesize($_FILES["fileToUploadInvestor"]["tmp_name"][$i]);
		if($check!==false)
		{
	...
	//check file size
	if($_FILES["fileToUploadInvestor"]["size"][$i]>500000)
	{
	...
	{
		if(move_uploaded_file($_FILES["fileToUploadInvestor"]["tmp_name"][$i], $target_file))
		{
			chmod($target_file,0777);
			echo "The file ". basename($_FILES["fileToUploadInvestor"]["name"][$i]). " has been uploaded.";
